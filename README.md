...
- Handles all business logic, data retrieval from the `company_data.csv` file, and machine learning computations.
...
- The backend URL is configurable in `frontend/app.py`.
...
-   `FINSIGHT/`
    ...
    -   `company_data.csv`: The raw dataset used to train the machine learning models and for live data retrieval.
    ...
    -   `migrate_db.py`: (Obsolete) A script to migrate data into a MySQL database. This is no longer needed as the backend reads directly from the CSV.
    -   `backend/`:
    ...
    -   `frontend/`:
        -   `app.py`: A Flask web server responsible for displaying HTML pages (templates) to the user. This file also contains the configuration for the backend URL.
...
3.  Before displaying the HTML page, the Frontend Server sends a `GET` request to the Backend API.
4.  The Backend API (`backend/app.py`) receives the `GET` request, retrieves unique data from the `company_data.csv` file (such as a list of countries, cities, categories), then sends it back to the Frontend Server in JSON format.
...
3.  JavaScript in the User's Browser sends a `POST` request directly to the corresponding Backend API endpoint (e.g., `/api/predict/profit`). The form data is sent in JSON format.
...
1.  **Configure Backend URL (Optional)**: If the backend is running on a different IP address or port, open `frontend/app.py` and modify the `BACKEND_URL` variable.
2.  **Run the backend**:
    - Open a new terminal.
...
-   `FINSIGHT/`
    ...
    -   `company_data.csv`: Dataset mentah yang digunakan untuk melatih model machine learning dan untuk pengambilan data langsung.
    ...
    -   `migrate_db.py`: (Usang) Skrip untuk memigrasikan data ke dalam database MySQL. Tidak lagi diperlukan karena backend membaca langsung dari CSV.
    -   `backend/`:
    ...
    -   `frontend/`:
        -   `app.py`: Server web Flask yang bertanggung jawab untuk menampilkan halaman HTML (template) kepada pengguna. File ini juga berisi konfigurasi untuk URL backend.
...
3.  Sebelum menampilkan halaman HTML, Server Frontend mengirimkan permintaan `GET` ke API Backend.
4.  API Backend (`backend/app.py`) menerima permintaan `GET`, mengambil data unik dari file `company_data.csv` (seperti daftar negara, kota, kategori), lalu mengirimkannya kembali ke Server Frontend dalam format JSON.
...
3.  JavaScript di Browser Pengguna mengirimkan permintaan `POST` langsung ke endpoint API Backend yang sesuai (misalnya, `/api/predict/profit`). Data formulir dikirim dalam format JSON.
